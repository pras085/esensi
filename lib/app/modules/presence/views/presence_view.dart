...
class PresenceView extends StatefulWidget {
  ...
  const PresenceView({Key key, @required this.cameraDescription})
      : super(key: key);

  @override
  PresenceViewState createState() => PresenceViewState();
}

class PresenceViewState extends State<PresenceView> {
  PageIndexController pageIndex = Get.find();
  String imagePath;
  Face faceDetected;
  Size imageSize;
  ...
  _onBackPressed() {
    Get.back(result: _faceNetService = null);
    pageIndex.changePage(0);
    pageIndex.isLoading.value = false;
  }
